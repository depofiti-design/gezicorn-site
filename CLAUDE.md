...
## 24 Eylül 2026 akşamı: İngiltere gönderisi, yeni yazılar, vize tablosu
- **Sosyal:** "İngiltere Schengen'e dahil mi?" 6 karelik carousel (IG `18108608474583150`, FB çoklu foto `144062395450039_122283452972056880`) + özel tasarım 1080x1920 hikaye (IG `18010081103768062`). Üretici: `social-content/apps/build_ingiltere.py` (gitignore'da klasör, görseller `img/social/ingiltere-*.jpg`). **Hikaye kuralı:** hikaye görseli her zaman 1080x1920 ayrı tasarım, metin üst 250 ve alt 340 px dışında; 16:9 kapak asla hikaye yapılmaz. FB carousel `post-social.js` `carousel_images` ile.
- **Yeni yazılar (Dışişleri ülke sayfasından doğrulandı):** `gurcistan-vizesi-turkler-icin-kimlikle-giris-sigorta` (1 yıl, çipli kimlik, 1 Ocak 2026'dan 30.000 GEL sigorta zorunlu, Abhazya yasak), `dubai-vizesi-turkler-icin-bordo-yesil-pasaport` (bordo vize gerekli, hususi/hizmet/diplomatik 90 gün muaf), `balkanlara-vizesiz-gidilir-mi-turkler-icin` (Bosna 180/90, Arnavutluk 6 ayda 90, Karadağ 180/30, Sırbistan 6 ayda 90). Kapaklar PIL ile `assets/3d/*.webp` nesnesi + gradyan, metinsiz. Kapaksız 2 yazıya kapak eklendi.
- **Vize tablosu:** `/vize-tablosu/` (scripts/build-vize-tablosu.js, veri `ROWS` dizisi, 24 ülke, arama + filtre). Veri değişince ROWS'u düzenle, `node scripts/build-vize-tablosu.js`, build-static, push. Ana sayfa hero CTA ve footer'da linkli, sitemap'te.
- **Mobil nav taşması** düzeltildi (`.gz-nav>nav{min-width:0}`), CSS_V `20260924b`.
- **Kırgızistan** yazısı: 180 günde 90 gün, visa-run yasağı (Bişkek Büyükelçiliği duyurusu). Malezya MDAC son 3 gün, Orta Asya güncellendi.
- **agy** ilk çağrıda dosya bulamayıp çok bekledi, sonra çalıştı: görev klasöründe dosya isteyen işlerde tam yol ver, çıktısı `~/.gemini/antigravity-cli/scratch/` altına düşebilir, çıktı doğrulanmadan yayınlanmaz.
- **Rakip analizi (agy, 7 site):** yapılmayanlar: filtrelenebilir vize matrisi (yapıldı), "Son güncelleme + resmi kaynak teyitli" rozeti, interaktif evrak checklist, renk kodlu dünya haritası, mesleğe göre evrak listeleri (çalışan, emekli, şirket sahibi), Schengen randevu taktikleri, dilekçe/sponsor mektubu şablonları, kapıda vize listesi, Mısır vizesi, Japonya Visit Japan Web rehberi, DTV/Bali dijital göçebe vizeleri. Sıradaki yazı fikirleri bunlardan.

## 25 Eylül 2026: "Pasaportunu seç" sayfası ve Kazakistan düzeltmesi
- **/vize-tablosu/** yeniden tasarlandı (passportindex tarzı): 1) bordo/yeşil pasaport seç, 2) vizesiz / e-vize-varışta / vize gerekli / (yeşilde) teyit gerekli sekmesi, arama, ülke kartları. Nav'ın ilk sekmesi "Vize Tablosu" (sarı, `.hl2`), ana sayfa hero butonu "Pasaportunu seç". Veri `scripts/build-vize-tablosu.js` `ROWS` (her ülke `pb` bordo, `py` yeşil). **Yeşil veri sadece T.C. Dışişleri ülke sayfasında görülenler için yazıldı (Gürcistan, Kırgızistan, Kazakistan, Tacikistan 60/180, Malezya, Vietnam 90, Japonya, Güney Kore K-ETA, Bosna, Arnavutluk, Sırbistan, BAE 90); gerisi `U` (teyit gerekli). Tahmin yazma.** Dışişleri URL kalıbı: `https://www.mfa.gov.tr/<ulke>-seyahat-edecek-turk-vatandaslarinin-dikkatine.tr.mfa` (Tayland/Hong Kong/Rusya/Kamboçya sayfaları bu kalıpla boş ya da 404).
- **KAZAKİSTAN DÜZELTME:** yazılarda "30 gün" yazıyordu, Dışişleri (2022 metni) ve Ticaret Bakanlığı "90 gün" diyor, bazı acenta kaynakları "giriş başına 30, 180 günde 90" diyor. 7 yazı, damga (`stamps.json`) ve ana sayfa rota durağı 90 gün yapıldı, yazıda "teyit et" notu var. Eski Instagram/Facebook "vizesiz ülkeler" carousel'inde Kazakistan 30 gün yazıyor olabilir, düzeltmeye gerek görülürse yeni gönderi at.
- CSS_V `20260925b`. Mobil nav taşması düzeltildi.

## 26 Eylül 2026: sosyal görsel formatı (kullanıcı kızdı, KALICI KURAL)
- Günlük otomasyon kapaksız yazıda `og-default.png`, kapaklı yazıda 16:9 kapağı doğrudan Instagram feed'e atıyordu (yatay çıktı). **Kural: Instagram feed 4:5 dikey (1080x1350), hikaye 9:16 (1080x1920), yatay görsel asla.** Facebook feed de aynı dikey görsel.
- Düzeltme: `scripts/social-image.py` (Pillow, `scripts/fonts/Bricolage.ttf`) her yazıdan feed + hikaye üretir, `scripts/daily-social.js` bunları `img/social/auto/` altına commit'leyip push'lar, sitede yayına girmesini bekler, sonra IG feed + FB + IG hikaye atar. Hikaye artık geri geldi (bu sefer 9:16 özel tasarım, güvenli alan üst 250/alt 340 px). Görsel üretilemezse paylaşım iptal olur.
- Test: `gh workflow run daily-social.yml -f force_slug=<slug> -f dry_run=true` sadece görsel üretir. `FORCE_SLUG=<slug> node scripts/daily-social.js` (yerelde) o yazıyı gerçekten paylaşır, `last_posted_slug` değişmez.
- 25 Eylül'de atılan yatay gönderi (`vize-evraklarinda-meslege-gore-farklar`) aynı gün dikey feed + hikaye olarak yeniden atıldı (IG `18018580085938677`, FB `144062395450039_122283638684056880`, hikaye `18132176815708107`). Eski yatay olanı kullanıcı siler.
- Elle hazırlanan gönderilerde de aynı kural: carousel 1080x1350, tek görsel 1080x1350, hikaye 1080x1920.
- **Insan görseli yok (26 Eylül 2026):** kullanıcı, yapay zekayla üretilmiş kendi benzerinin olduğu eski gönderileri "çok kötü" buldu. Yeni sosyal görsellerde üretilmiş insan/benzer yok, sadece tasarım grafiği (tipografi, 3D nesne) ya da kullanıcının verdiği gerçek fotoğraf. Instagram profil ızgarası 3:4 kırpar, metni kenardan ≥70 px içeride tut.
- **Carousel şablonu (26 Eylül 2026, kullanıcı "logo başlık hizası diğerleriyle aynı olsun, kaydırmalıya özen göster" dedi):** elle hazırlanan tüm kaydırmalı gönderiler `scripts/social-carousel.py` ile üretilir (spec JSON: cover, slides[], outro, story). Sabit düzen: sol üstte logo + "gezicorn" yan yana, sol kenar 70 px, sağ üst boş (IG rozeti kapatır), sayaç sol altta, "kaydır" sağ altta, altta ortalanmış sarı daire + 3D nesne (`assets/3d`), aynı yazı boyutları. Hikaye aynı spec'in `story` bölümünden 1080x1920. İngiltere carousel (24 Eylül) eski düzendeydi (marka yazısı sağ üstte), yenisiyle uyumsuz.
- **e-Arrival** gönderisi bu şablonla yeniden atıldı (IG carousel `18115160327000726`, FB çoklu foto `144062395450039_122283640064056880`, hikaye `18166896349463541`). Eski yatay gönderiyi kullanıcı sildi. Makalenin kapağı da yapay zeka insan görseli yerine tasarım (telefon nesnesi) kapağıyla değiştirildi.
