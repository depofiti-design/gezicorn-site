...
1. **Yazım/dil hatası taraması: 20 Eylül 2026 tamamlandı** (78 yazının tamamı okundu, 39 yazı düzeltildi; deals, rota önerileri, sosyal linkler ve ana sayfa/danışmanlık metinleri tarandı). Düzeltilenler: bozuk başlıklar (`turklerin-en-sevdigi-vizesiz-rotalar`, `neden-gezmeli-rotasiz-hayat`), sen/siz karışıklığı, ret/red, imkân/bekâr, check-in, Rusya kart cümleleri, danışmanlık sayfasındaki "süren göre", Tayland rota notundaki "kuru sezon". Eski site duyuruları `yakinda-kamp-gezi-urunu-firsatlari` ve `dunya-haritasi-yenilendi` yayından kaldırıldı (`published:false`, eski tasarıma ve boş fırsat bölümüne atıf yapıyordu). `konsoloslukta-randevu-beklerken-ogrendiklerim` yazısındaki kişisel anlatı (uydurma olabilir) nötr anlatıma çevrildi, başlık "Konsolosluk Randevusu Beklerken Bilinmesi Gerekenler" oldu (slug aynı). Kullanıcıdan teyit bekleyenler: `yurt-disinda-uzun-sureli-kiralik-ev-nasil-bulunur` ("ilk hafta turistik bölgede kalıp yerel mahalleye taşındım"), `neden-gezmeli-rotasiz-hayat` ("Bişkek'e ilk geldiğimde tek kelime Rusça bilmiyordum") ve Kamboçya/Kırgızistan yazılarındaki "yaşayan biri olarak" cümleleri gerçek deneyim mi. Ayrıca `kamboca-e-vize-basvurusu-adim-adim` ile `kambocya-vizesi-turkler-icin-e-vize-sureci`, `vize-mulakatina-nasil-hazirlanilir-10-ipucu` ile `vize-basvurusunda-mulakat-sureci-nasil-isliyor` konu olarak çok yakın (Google'da birbirini yiyebilir), birleştirme ya da ayrıştırma düşünülebilir. `kategori-sayfalari-eklendi` (noindex) "ana sayfa son 3 yazıyı gösteriyor" diyor, güncel mi kontrol edilmeli.
